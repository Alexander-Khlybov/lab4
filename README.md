...
Приоритетная очередь содержит нижеизложенные public-методы:
...
![disjoint_set](/Rept/disjoint_set.png)
...
###Представление графа
Граф представляется набором ребер (multiset<EDGE>).

Секция public содержит следующие методы:
...
При использовании класса *GRAPH* возможны следующие исключительные ситуации:
- `GRAPH(size_t)` - "Out of range", если количество вершин графа превышает значение константы MAX_VERTICE_NUM;
- `setDistance(size_t, size_t, double)` - "Out of range", если первый или второй параметр больше, чем количество ребер графа;
- `getDistance(size_t, size_t)` - "Out of range", если хотя бы один из параметров больше, чем количество ребер графа;
- `getSetOfEdges(size_t)` - "Out of range", если значение вершины, переданной в качестве параметра больше количества вершин;
- `fillGraph(void)` - наложение исключения из setDistance;
